...
const post = async <TData, ECode extends string>(
  path: string,
  body: unknown,
): Promise<ApiResponse<TData, ECode>> => {
...
export const requestPasswordReset = async (command: PasswordResetRequestCommand) => {
  const json = await post<
    PasswordResetRequestResponse["data"],
    PasswordResetRequestErrorCode
  >(`${AUTH_BASE_PATH}/password-reset/request`, command)
...
export const confirmPasswordReset = async (command: PasswordResetConfirmCommand) => {
  const json = await post<
    PasswordResetConfirmResponse["data"],
    PasswordResetConfirmErrorCode
  >(`${AUTH_BASE_PATH}/password-reset/confirm`, command)
