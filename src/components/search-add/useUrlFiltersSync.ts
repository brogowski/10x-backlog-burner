...
type FiltersUpdater =
  | Partial<SearchFiltersVM>
  | ((prev: SearchFiltersVM) => Partial<SearchFiltersVM>)
...
export const useUrlFiltersSync = (initial?: Partial<SearchFiltersVM>) => {
  ...
  const updateFilters = useCallback(
    (updater: FiltersUpdater, options?: UpdateOptions) => {
      setFilters((prev) => {
        const partial =
...
const mapQueryToViewModel = (
  query: ReturnType<typeof parseGamesSearchParams>,
): SearchFiltersVM => ({
  search: query.search ?? "",
  genres: query.genres ?? [],
...
const sanitizeFilters = (filters: SearchFiltersVM): SearchFiltersVM => {
  ...
  const sort = allowedSorts.includes(filters.sort)
    ? filters.sort
  ...
  const genres = dedupe(
    (filters.genres ?? []).map((genre) => genre.trim()).filter(Boolean),
  )
  ...
  const hasConflict =
    releasedAfter &&
    releasedBefore &&
    Date.parse(releasedAfter) > Date.parse(releasedBefore)
