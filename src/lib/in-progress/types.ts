...
export type InProgressGameItemVM = {
  steamAppId: number
  title: string
...
export type ReorderInProgressRequest = {
  items: { steamAppId: number; position: number }[]
}

export type ReorderInProgressResult = {
  updated: number
}
...
export type UpdateUserGameStatusRequest = {
  status?: GamePlayStatus
...
export const mapUserGamesToQueue = (dto: UserGamesListDTO): InProgressQueueVM => {
  ...
  const items = sorted.map((item, index): InProgressGameItemVM => ({
