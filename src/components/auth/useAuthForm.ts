...
export const useAuthForm = (
  ...
  const initialValues = useMemo(() => (mode === "login" ? defaultLoginValues : defaultSignupValues), [mode]);
  const [values, setValues] = useState<AuthFormValuesLogin | AuthFormValuesSignup>(initialValues);
  const [errors, setErrors] = useState<Partial<Record<keyof typeof values | "general", string>>>({});
