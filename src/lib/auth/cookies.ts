...
export const setCookie = (headers: Headers, name: string, value: string, options: CookieOptions = {}) => {
...
export const appendSessionCookies = (headers: Headers, session: Session) => {
  const accessExpiresAt = session.expires_at !== undefined ? new Date(session.expires_at * 1000) : undefined;
