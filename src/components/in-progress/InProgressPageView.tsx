...
const InProgressPageView = () => {
  ...
  const capState = useMemo<CapState>(
    () => ({
      ...
      notice: queue && queue.isAtCap ? "You’ve reached the in-progress cap. Finish a game to add another." : undefined,
    }),
    [queue]
  );
    ...
    <section className="mx-auto flex max-w-4xl flex-col gap-6 px-6 py-10">
      <header className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm font-medium uppercase tracking-wide text-foreground/70">In-progress queue</p>
