...
const log = {
  warn: (message: string) =>
    console.warn(`[Playwright teardown] ${message}`),
  error: (message: string, error?: unknown) =>
    console.error(`[Playwright teardown] ${message}`, error ?? ""),
...
const signInForCleanup = async (
  ...
  if (!data.user) {
    log.warn(
      "Supabase returned no user after signin; skipping backlog cleanup.",
    )
    return null
  }
