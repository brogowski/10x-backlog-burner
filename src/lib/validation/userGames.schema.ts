...
export const parseUserGamesQuery = (params: URLSearchParams): UserGamesQuery => {
  ...
  const onlyInProgress =
    ...
    parsed.statuses.every((status) => status === "in_progress")
...
export const parseCreateUserGame = (payload: unknown): CreateUserGamePayload =>
  createUserGameSchema.parse(payload)
